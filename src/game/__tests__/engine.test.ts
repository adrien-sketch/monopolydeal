...
function makeTestState(): GameState {
  ...
  // Override with deterministic state for testing
...
describe('drawCards', () => {
  it('draws 5 cards on first turn', () => {
    const state = createInitialState()
    const s = drawCards(state)
    expect(s.players.human.hand).toHaveLength(10) // 5 initial + 5 drawn
    expect(s.turnPhase).toBe('play')
    expect(s.isFirstTurn.human).toBe(false)
  })
...
describe('resolveDiscard', () => {
  it('removes cards from hand and switches turn', () => {
    const state = makeTestState()
    const s = drawCards(state)
    while (s.players.human.hand.length < 10) {
      s.players.human.hand.push({ id: `extra-${s.players.human.hand.length}`, type: 'money', name: '1M', bankValue: 1 })
...
describe('win condition', () => {
  it('detects 3 complete sets as a win', () => {
    const state = makeTestState()
    const s = drawCards(state)
